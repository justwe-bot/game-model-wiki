"""Validate Bandit's original Unity rig, weights, and animation clips."""
...
EXPECTED_ANIMATIONS = {
    "Bandit_SelectPoseIdle",
    "Bandit_RunForward",
    "Bandit_RunBackward",
    "Bandit_RunLeft",
    "Bandit_RunRight",
    "Bandit_FireMainWeapon",
    "Bandit_Reload",
    "Bandit_SlashBlade",
}
...
def validate(path: Path) -> dict[str, int | float]:
    ...
    if len(skin["joints"]) != 98:
        raise ValueError(f"{path}: expected 98 original joints, found {len(skin['joints'])}")
