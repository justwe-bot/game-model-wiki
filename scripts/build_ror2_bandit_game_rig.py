"""Build Bandit GLBs with the original Unity skeleton, weights, and clips."""
...
from ror2_bandit_animation_curves import CLIPS, parse_rotation_curves, quat_normalize
...
def add_game_animations(
    ...
    for spec in CLIPS:
        ...
        if not channels:
            raise ValueError(f"{source_path}: no curves matched the original Bandit hierarchy")
        animations.append(
            {
                "name": spec.name,
                "samplers": samplers,
...
def build_original(
    ...
    root = find_model_root(environment, ["Bandit2BodyMesh"], "mdlBandit2@MasterAnims")
    ...
    document = {
        "asset": {
            "version": "2.0",
            "generator": "ror2-wiki Bandit original-rig builder",
            "extras": {
                "rig": {
    ...
    skinned_renderers = []
    static_renderers = []
    excluded = {"BanditShotgunMesh", "BanditPistolMesh"}
    ...
    if not skinned_renderers:
        raise ValueError("Bandit prefab has no skinned renderers")
    ...
    for transform, game_object, mesh, renderer in skinned_renderers:
        ...
        if renderer_keys != joint_keys:
            raise ValueError(f"{game_object.m_Name}: bone ordering differs from Bandit body")
...
def main() -> None:
    ...
    print(
        f"Built original Bandit rig: {result['sourceTriangles']} -> {result['lowTriangles']} triangles, "
