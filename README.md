# Game Model Wiki

面向多个游戏的轻量 3D 模型 Wiki。当前第一个可用模块是《雨中冒险 2》，模型来自本机 Steam 安装并经过移动端保形处理。

项目暂时采用单游戏页面，后续会按 `games/<game-slug>/` 拆分目录，并在根页面提供游戏选择；共用 Three.js 查看器、移动端加载策略和模型信息结构，避免每增加一个游戏就复制整套代码。

## 当前游戏

- `risk-of-rain-2`：20 个怪物和 23 张地图结构预览

## 当前内容

- 20 个可浏览怪物，均包含游戏原始模型和移动端保形低模 GLB
- 保留 UV、原始配色、骨骼和可用动画
- 技能卡片可播放对应攻击动作，并提供中文攻击形式、距离和说明
- 支持搜索、体型筛选、原模/低模/并排对比、触控旋转和缩放
- 适配桌面与手机浏览器
- 手机端默认只加载当前条目的低模；普通模式同时保留 1 个模型，对比模式最多 2 个
- 手机端使用 1 倍渲染像素比，画布离开屏幕或标签页隐藏时暂停渲染
- 石傀儡使用独立发光贴图显示红眼；黏土圣堂武士接入法线贴图以恢复炮体和护甲细节
- 独立 `maps.html` 地图页支持搜索、扩展包筛选、自动旋转、俯视和重置视角
- 地图从 Unity 静态组合网格生成并使用顶点高度配色；一次只加载当前地图 GLB
- 地图页手机端使用 1 倍像素比、无光照顶点色材质，并在画布离屏时暂停渲染

低模使用误差受限简化。10% 只是期望目标，不会为了达到固定比例强行删除脚、细腿、角、炮口或其他关键轮廓。不同模型会在接近明显变形前停止，因此实际保留比例约为 23%-79%。

低模贴图使用 256x256、96 色 RGB 像素化处理，并保留 UV；透明模型会单独保留 Alpha。页面采用最近邻采样和平面着色强化低多边形观感。

## 本机预览

在此目录启动静态文件服务器：

```powershell
python -m http.server 4174 --bind 127.0.0.1
```

- 生物页：`http://127.0.0.1:4174/`
- 地图页：`http://127.0.0.1:4174/maps.html`

## 模型规模

| 怪物 | 原模三角面 | 低模三角面 | 保留比例 |
| --- | ---: | ---: | ---: |
| 甲壳虫 | 4,560 | 2,566 | 56% |
| 利莫里亚 | 3,586 | 2,836 | 79% |
| 石傀儡 | 4,048 | 2,674 | 66% |
| 小恶魔 | 2,436 | 1,362 | 56% |
| 巨角野牛 | 6,864 | 5,358 | 78% |
| 巨型幽魂 | 2,508 | 1,930 | 77% |
| 甲壳虫女王 | 17,944 | 5,317 | 30% |
| 黏土沙丘行者 | 12,104 | 9,492 | 78% |
| 甲壳虫守卫 | 13,080 | 4,298 | 33% |
| 远古利莫里亚 | 8,808 | 5,147 | 58% |
| 黏土圣堂武士 | 9,928 | 6,978 | 70% |
| 寄居蟹 | 9,034 | 5,718 | 63% |
| 水母 | 1,518 | 1,166 | 77% |
| 游荡者 | 7,270 | 3,620 | 50% |
| 小恶魔霸主 | 8,904 | 4,461 | 50% |
| 石巨人 | 15,526 | 9,560 | 62% |
| 合金秃鹫 | 8,146 | 1,876 | 23% |
| 黄铜装置 | 2,742 | 2,162 | 79% |
| 月球傀儡 | 9,118 | 5,270 | 58% |
| 月球幽魂 | 2,852 | 2,100 | 74% |

## 验证状态

- 20 个原模和 20 个低模均包含有效网格和三角面
- 所有低模均保留 UV；需要骨骼的网格保留蒙皮属性
- 每个条目的默认动作和技能动作都存在于原模与低模 GLB
- 月球傀儡与月球幽魂的三项技能动作已在浏览器中逐项播放验证
- 甲壳虫女王、巨角野牛等细肢模型已用并排对比检查，低模未再出现缺脚问题
- 23 个地图 GLB 均包含有效网格、三角面和 `COLOR_0` 顶点色
- 地图页已抽查泰坦平原、深渊深处、月球开端、铁质冲积地和再利用环形坑的加载与自动取景

## 已知边界

- 技能按钮播放的是模型骨骼动作。火球、激光、火焰、光晕、冲击波等独立粒子/VFX 不包含在角色 GLB 中。
- 少数游戏资源把静态网格和动作骨架拆在不同包中，需要额外的骨架与蒙皮合并管线，当前不会用空骨架冒充完整模型。
- 巨型幽魂与黏土沙丘行者的 FBX 导出缺少部分蒙皮权重，构建管线会为缺失顶点补充刚性权重，适合 Wiki 预览，但不等同于游戏项目中的精确蒙皮数据。
- 目录中保留了早期突击兵英雄验证文件，英雄尚未接入当前怪物列表。
- 地图结构预览不包含原游戏材质、植被实例、粒子、天空盒、动态机关和烘焙光照；极端纵深会适度压缩以便查看。
- 破碎林地、重塑祭坛、树生殖民地、金色凋亡、蠕虫孵化场和导管峡谷的场景包只导出零散运行时片段，当前不展示。
- 虚空之境等运行时拼装场景没有可用静态网格；少数损坏的 OBJ 需要后续改用场景模块重建流程。

## 素材说明

游戏模型和贴图的权利归原权利方所有。这个目录适合作为本机研究、个人原型和技术验证；公开发布或提供下载前，请核对游戏 EULA、商标和素材使用许可，不要分发原始 bundle、FBX 或原尺寸贴图。
